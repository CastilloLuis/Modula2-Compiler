...
const regEx = {
    ...
    'varDeclaration': /^(VAR|CONST)\s([A-Za-z]+)\s(:)\s(INTEGER|CHAR|BOOLEAN|FLOAT);$/,
    'checkVar': /^(var)/i,
    'checkFunction': /^(BEGIN)\n(.*?(\n))+.*?\n(END)\s([A-Za-z]+)\.$/, // 
...
const compiler = () => {
    try {
        ...
        let declaredVars = [];
        let assignedVars = [];
        if (syntax.includes('BEGIN')) { // SI ES DECLARACION DE VARIABLES + LA FUNCIÓN
            ...
            declaredVarsOk = (l.every(checkArray)); // CHECK IF ALL THE VARS WERE CORRECTLY DECLARED     
            if (declaredVarsOk) {
            ...
            ((declaredVarsOk) ? compilerOutput(true, 'VARIABLES DECLARADAS CORRECTAMENTE') : compilerOutput(false, 'CHECK DECLARED VARS'));
        }
